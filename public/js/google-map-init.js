...
function googlemap_init(dom_obj, coords) {
  ...
  try {
    ...
    THEMEMASCOT_googlemap_init_obj[id].opt = {
      ...
      styles:
        THEMEMASCOT_googlemap_styles[coords.style ? coords.style : "default"],
      mapTypeId: google.maps.MapTypeId.ROADMAP,
    };

    var custom_map = new google.maps.Geocoder();
    custom_map.geocode(
      coords.latlng !== "" ? { latLng: latlng } : { address: coords.address },
      function (results, status) {
        if (status == google.maps.GeocoderStatus.OK) {
          THEMEMASCOT_googlemap_init_obj[id].address =
    ...
    jQuery(window).resize(function () {
      if (THEMEMASCOT_googlemap_init_obj[id].map) {
        THEMEMASCOT_googlemap_init_obj[id].map.setCenter(
          THEMEMASCOT_googlemap_init_obj[id].address
        );
      }
    });
...
function googlemap_create(id) {
  ...
  THEMEMASCOT_googlemap_init_obj[id].map = new google.maps.Map(
    THEMEMASCOT_googlemap_init_obj[id].dom,
    THEMEMASCOT_googlemap_init_obj[id].opt
  );
  THEMEMASCOT_googlemap_init_obj[id].map.setCenter(
    THEMEMASCOT_googlemap_init_obj[id].address
