...
exports.getOrderproductById=async (req, res,next) => {
  try {
    const {id}=req.params
    if(isUserAdmin(req)){
    const order_product = await Orderproduct.findOne({where:{id},
       include: ["order"] });
    return res.json(order_product);
    }
    const order_product= await Orderproduct.findOne({where:{id},
      where:{'$Order.userId$':req?.user?.sub}, include: 
      ["order"] });
   return res.json(order_product);
  } catch (err) {
   next(err)
...
exports.editOrderproduct=async (req, res,next) => {
  try {
    const { id } = req.params;
    if(isUserAdmin(req)){
    const updated_order_product = await Orderproduct.update({...req.body},
      {where:{id:id}});
    return res.json(updated_order_product);
    }
    const change={}
    if(req.body.quantity){change.quantity=req.body.quantity}
    const updated_order_product = await Orderproduct.update({...change},
